            ...
            <table className="notices-table">
              ...
              <tbody>
                {notices.map((notice, index) => (
                  <tr key={notice._id}>
                    <td style={{ textAlign: 'center', fontWeight: 600, color: '#6A1B9A' }}>{index + 1}</td>
                    <td>
                      ...
                      <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                        {(notice.pdfUrl || notice.imageUrl) && (
                          <a
                            ...
                            target="_blank"
                            style={{ padding: '5px 14px', background: '#6A1B9A', color: '#fff', fontSize: '12px', fontWeight: 600, borderRadius: '4px', textDecoration: 'none', cursor: 'pointer' }}
                          >View</a>
                        )}
                        {(notice.pdfUrl || notice.imageUrl) && (
                          <a
                            href={notice.pdfUrl ? `${API_URL}/pdf-proxy?url=${encodeURIComponent(notice.pdfUrl)}` : notice.imageUrl}
                            download
                            style={{ padding: '5px 14px', background: '#fff', color: '#6A1B9A', fontSize: '12px', fontWeight: 600, borderRadius: '4px', border: '1px solid #6A1B9A', textDecoration: 'none', cursor: 'pointer' }}
                          >Download</a>
                        )}
                      </div>
                    </td>
