        ...
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          ...
          {value && (
            <>
              ...
              <a href={value} target="_blank" rel="noopener noreferrer" style={{ color: '#6A1B9A', fontWeight: 600, fontSize: '11px', textDecoration: 'none' }}>View</a>
              <button
                type="button"
      ...
      {value ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', background: '#f8f9fa', border: '1px solid #e4e8ed', borderRadius: '6px' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <a href={`${API_URL}/pdf-proxy?url=${encodeURIComponent(value)}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '12px', color: '#6A1B9A', textDecoration: 'none' }}>View file →</a>
          </div>
          <button
            type="button"
            onClick={handleRemove}
        ...
        >
          {uploading ? (
            <div>
              <div style={{ fontSize: '13px', color: '#6A1B9A', fontWeight: 600 }}>Uploading... {progress}%</div>
              <div style={{ marginTop: '8px', width: '100%', height: '4px', background: '#e4e8ed', borderRadius: '2px', overflow: 'hidden' }}>
                <div style={{ width: `${progress}%`, height: '100%', background: '#6A1B9A', borderRadius: '2px', transition: 'width 0.3s' }}></div>
              </div>
            </div>
          ) : (
            <>
